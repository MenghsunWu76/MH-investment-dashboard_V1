...
# === 標題 ===
st.title("🛡️ 全天候動態曝險戰情室 (v6.0 甜甜圈版)")
st.caption("核心：MDD 階梯加碼 + 閥值再平衡 (+/- 3%) + 雙重風控")
...
with c1:
    ...
    colors = {
        '攻擊型 (正二)': '#FF4B4B',  # Red
        '核心 (00662)': '#FFD700',   # Gold
        '防禦 (00713)': '#2E8B57',   # SeaGreen
        '子彈庫 (債券)': '#87CEFA'   # LightSkyBlue
    }
    ...
    fig = px.pie(
        ...
        hole=0.45,  # 設定中空大小 (甜甜圈)
    )
    
    # 4. 調整顯示設定 (文字在內部，顯示百分比)
