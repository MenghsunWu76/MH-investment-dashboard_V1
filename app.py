...
with st.sidebar:
    ...
    with st.expander("0. 市場位階 (ATH 修正)", expanded=True):
        ...
        with col_ath2:
            use_manual_ath = st.checkbox("手動修正", value=False)
...
if last_record is not None:
    ...
    diff_net = net_assets - last_record['Net_Assets']
...
with st.sidebar:
    ...
    if last_record is not None:
        ...
        st.caption(f"上次市值: ${last_record['Total_Assets']:,.0f}")
...
col1.metric(
    "💰 資產總市值 (I)", 
    f"${total_assets:,.0f}", 
    delta=f"${diff_total:,.0f} (vs 上次)", 
    help=f"上次紀錄時間: {last_date_str}"
)
